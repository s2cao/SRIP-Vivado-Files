...
int main()
{
  ...
  xil_printf("UART Lite Tx test successful\n\r");
  xil_printf("attempting receive\n\r");
//  RxData[1] = XUartLite_Recv(&UartLite0, RxBuff, 1); //not needed
  u8 user_input;
  user_input = XUartLite_RecvByte(XPAR_AXI_UARTLITE_0_BASEADDR);
  xil_printf("the keystroke is: %c\n\r", user_input);
  xil_printf("the character in ASCII is: %u\n\r", user_input);
  xil_printf("receive finished\n\r");
...
//int ReceiveData(u8 *RxDataPointer, u8 *RxDataBufferPointer) {
//	XUartLite_Recv(&UartLite0, RxDataBufferPointer, 1)
//}
